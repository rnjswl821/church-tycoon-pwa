...
const AUTO_PLAY_INTERVAL_MS = 15 * 1000; // 15초 = 게임 속 1주(오너 지시 — 베타 피드백 "자동진행 너무 느림" 반영)
...
const OFFLINE_SECONDS_PER_WEEK = 15; // 실시간 자동진행(15초=1주)과 같은 속도로 맞춤 — 앱을 꺼두는 쪽이 오히려 손해 보지 않도록
/* 웹페이지는 탭이 닫히거나 백그라운드로 가면 실제로 JS가 돌지 않는다(브라우저 표준 제약 —
   ...
   건너뛰는 극단적 결과를 막는다. */
const OFFLINE_MAX_WEEKS = 5760; // 15초/주 기준 24시간(하루) 분량 — 속도가 바뀌어도 최대 인정 시간(하루)은 그대로 유지
...
const UPDATE_LOG = [
  { date: '2026-09-02', title: '아이콘 전체 화질 개선', items: [
    '아이콘 117개 전부를 더 매끈하고 입체감 있게 다시 그렸습니다',
    '가장자리가 또렷하고 부드럽게 표현됩니다',
    '위는 밝고 아래는 살짝 어두운 음영으로 평면적인 느낌을 줄였습니다',
  ] },
